              ...
              <div className="grid grid-cols-7 gap-y-3 sm:gap-y-4 gap-x-2 justify-items-center">
                ...
                {medicationGrid.slice(medicationMonth * 30, (medicationMonth + 1) * 30).map((day, i) => (
                  <div
                    key={i}
                    title={`Dia ${i + 1}`}
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg transition-transform hover:scale-105 cursor-pointer shadow-sm flex items-center justify-center text-[10px] sm:text-xs font-bold text-black/30 border border-black/5 ${day === 'green' ? 'bg-[#22c55e]' : day === 'yellow' ? 'bg-[#facc15]' : 'bg-[#ef4444]'
                      }`}
                  >
                    {i + 1}
                  </div>
